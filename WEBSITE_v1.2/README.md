...
<h1 align="center">HYDRA SAMO — Production Website (v1.2)</h1>
...
This folder is the **evolving production website** (`WEBSITE_v1.2`), cloned
verbatim from the frozen `WEBSITE_v1.1` tree with every asset preserved as-is.

`WEBSITE_v1.1` is the frozen shipped tree (read-only by convention) that GitHub
Actions currently builds and deploys to GitHub Pages — it is not edited. This
v1.2 tree is the workspace where the next iteration happens; when it is ready,
the deploy pipeline is switched to build this folder instead.
...
full layout (`WEBSITE_v1.1` frozen / `WEBSITE_v1.2` evolving / `BRAND`).
