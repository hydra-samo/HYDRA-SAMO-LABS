...
import { Play, ArrowRight, ArrowLeft, Film } from 'lucide-react';
...
// Line-by-line section header reveal — each word rises from y:40 on its own
// beat so the header assembles instead of fading in as one flat block.
...
export const ProjectCard = React.memo(function ProjectCard({
  ...
  const ctaArrowClass = dir === 'rtl' ? 'group-hover:-translate-x-1' : 'group-hover:translate-x-1';
            ...
            >
              ...
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 group-active:opacity-100 transition-opacity duration-300 z-20">
                <div className="w-16 h-16 rounded-full bg-accent text-white hover:bg-accent-dark dark:bg-white/[0.05] dark:border-emerald-500/30 dark:text-white dark:hover:bg-white/10 flex items-center justify-center shadow-lg transform scale-75 group-hover:scale-100 group-active:scale-100 transition-transform duration-300">
                  <Play size={24} className="fill-current text-accent ml-1 rtl:ml-0 rtl:mr-1" />
                </div>
              </div>
            </motion.div>
          )}
      ...
      <div className="p-6 relative z-10">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-slate-200 dark:border-white/10">
          ...
          <div className="flex items-center gap-1 text-xs font-semibold text-accent transition-colors">
            ...
            <CTAArrow size={14} className={`transition-transform ${ctaArrowClass}`} />
          </div>
        </div>
        </div>
      </TiltCard>
    ...
    <section id="work" className="py-8 sm:py-10 md:py-24 px-4 sm:px-6 relative transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div
          ...
          className="relative z-20 flex flex-col md:flex-row md:items-end justify-between mb-6 md:mb-12 gap-6 md:gap-8"
        >
          <div>
            <motion.h2
              initial="hidden"
              ...
              className="text-3xl sm:text-4xl md:text-5xl font-semibold tracking-tight uppercase font-display text-[var(--text-main)]"
            >
