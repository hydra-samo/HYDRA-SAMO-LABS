...
Every website asset is derived from the certified V4 mark. The live geometry lives in `src/components/HydraLogo.tsx`; static web assets live in `/public`. All lighting stays external (CSS layer) — never baked into the SVG.
...
|---|---|---|---|
| **Navbar** | `src/components/Navigation.tsx` | `HydraLogo` component | Emerald `#10b981` via `text-accent` |
| **Hero** | `src/components/Hero.tsx` | `HydraLogo` component | Emerald `#10b981` |
...
| **OpenGraph** | `public/hydra_logo.jpg` via `useOpenGraph.ts` | 1024×1024 raster | Emerald on Abyssal `#060c09` |
| **Twitter Card** | `public/hydra_logo.jpg` (same file) | `summary_large_image` | Emerald on Abyssal |
...
- **PreSplashSelector:** the mark as a selectable identity tile; emerald fill, glassmorphic tile per DESIGN.md component rules.
...
|---|---|---|---|
| Browser favicon (SVG) | `/hydra-mark.svg` (served) | vector | Referenced in `index.html` as `%BASE_URL%hydra-mark.svg` |
...
- **Metadata source:** `metadata.json` → injected by `src/hooks/useOpenGraph.ts`.
...
| File | Source | Status |
|---|---|---|
| `src/components/HydraLogo.tsx` | Master geometry | ✅ live |
| `public/hydra-mark.svg` | Derived (identical geometry) | ✅ live |
| `public/hydra_logo.jpg` | Derived (1024×1024 social card) | ✅ live |
