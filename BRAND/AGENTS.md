...
- **Imports**: Use **RELATIVE IMPORTS ONLY** (e.g. `../components/...`). The `@` alias points to root, not `WEBSITE_v1.1/src/`. NEVER use `@/components/`.
- **Styling**: Tailwind v4 CSS-first config in `WEBSITE_v1.1/src/index.css` (`@import "tailwindcss"`). Dark mode is class-based (`html.dark`), controlled by `App.tsx` and `localStorage['hydra-theme']`. Always use `cn()` from `WEBSITE_v1.1/src/lib/utils.ts` for conditional class logic.
...
- **Data Source**: All content comes from `WEBSITE_v1.1/src/data/portfolioData.ts`.
...
- **Images**: Local portraits live in `WEBSITE_v1.1/src/assets/images/` (e.g. `hydra_samo.webp`). Shared site assets live in `/public/`.
...
- **Icon only**: The Hydra brand mark (`WEBSITE_v1.1/src/components/HydraLogo.tsx`) must never contain text, initials, typography, slogans, or borders. Prohibit adding any of these in future refactors — the mark is a single, standalone icon.
...
- **External lighting**: All lighting, glow, and reflections must remain external in the CSS layer utilities (`.hydra-mark-glow`, `.hydra-mark-pulse` in `WEBSITE_v1.1/src/index.css`). Never bake gradients, glow, shadows, or reflections into the SVG itself — keep the raw vector clarity intact. Continuous rotation (`.hydra-mark-spin`) is retired: the 3-fold mark is rotationally self-similar, so spinning reads as a generic fan and is forbidden.
